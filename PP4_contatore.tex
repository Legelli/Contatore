...
\begin{tabular}{cccc}
\toprule
Intervallo [\SI{}{s}] & $\chi^2$ & $\nu$ & p\\
\hline 
$\Delta t_1 = 1$ &  3.880 & 4 & 0.42\\  
$\Delta t_2 = 2$ & 1.499 & 6& 0.96\\
$\Delta t_3 = 3$ & 6.705 & 6& 0.35\\
$\Delta t_4 = 4$ & 9.716 & 6& 0.13\\
$\Delta t_5 = 10$ & 13.214 & 11&  0.28\\
\bottomrule
\end{tabular}
\end{table}

I risultati del test mostrano che i dati sperimentali sono compatibili con il modello di una poissoniana: l'ipotesi nulla si sarebbe dovuta scartare nel caso di un valore di $p$ minore o uguale dello $\SI{0.05}{}$, nel nostro caso i risultati sono tutti distanti da questa soglia. Il risultato migliore si ottiene nell'intervallo di $\SI{2}{s}$: guardando anche la sovrapposizione della curva sull'istogramma (figura \ref{fig:istogramma_deltat2}) si nota come i dati sperimentali si adattino particolarmente bene alla curva teorica.
...
\end{equation}
dove con $X$ indichiamo l'insieme dei dati sperimentali relativi ai conteggi.
Poiché tale funzione risulta assumere valori non distinguibili da $0$ dal calcolatore, si riporta nel grafico di figura \ref{fig:verosimiglianzacombinata}, a meno di fattori di normalizzazione, il logaritmo naturale della funzione di verosimiglianza, 
\[
\log\big(f(r\vert X)\big) \propto 1022\log(r) - 1000r.
\]
Secondo il criterio di massima verosimiglianza otteniamo che il punto di massimo della funzione precedente corrisponde alla nostra stima di $r_\text{best}$, ossia $r_\text{best} = \SI{1.02 \pm 0.03}{s^{-1}}$.


\paragraph{Frazione di 0 e 0+1 conteggi} 
Escludendo il set di dati relativo a $\Delta t_5$, calcoliamo la frazione di $0$ conteggi e di $0$ o $1$ conteggio, riportata in tabella~\ref{tab:0e01conteggi}. Introduciamo il parametro $\tau = 1/r_{\text{best}}$ indicante la previsione del tempo di attesa prima che si verifichi il primo conteggio. Per quanto riguarda la prima frazione, essa ci permette di stimare l'andamento della probabilità  che il primo conteggio si verifichi dopo un intervallo di tempo $\Delta t$, ed equivale alla differenza tra $1$ e la funzione di ripartizione del tempo di attesa per un conteggio, ossia
\begin{equation}\label{eq:frazione0}
F(\Delta t) = e^{-\frac{\Delta t}{\tau}} = e^{-r_{\text{best}}\Delta t}.
\end{equation}
Sovrapponiamo dunque al grafico, in figura~\ref{fig:0conteggi}, delle frazioni di $0$ conteggi in funzione di $\Delta t$ l'andamento aspettato previsto dalla distribuzione esponenziale dei tempi di attesa, descritto dalla equazione precedente, dove abbiamo utilizzato come \emph{rate} quella estratta precedentemente, ossia $r_{\text{best}}$. Come incertezza notiamo che l'errore qui associato è di tipo binomiale, poiché consideriamo come \emph{successo} il numero di conteggi pari a $0$ e \emph{insuccesso} l'evento complementare. Pertanto, si ha
...
\]
e ricaviamo $\chi^2 = 4.51$. Considerando (per quanto già detto sopra) un numero di gradi di libertà pari a $\nu = 4$ concludiamo che entrambe le frazioni sono compatibili con il modello descritto da (\ref{eq:frazione0}) e (\ref{eq:frazione}).
...
Supponendo realisticamente invece che la probabilità non sia equivalente (per via di eventuali inclinazioni, per esempio) si avrà  una distribuzione non simmetrica descritta da una binomiale $\mathscr{B}_{N, p}$ dove la probabilità è il valore atteso della distribuzione delle frequenze relative, cioè
...
\includegraphics[scale=0.5]{verosimiglianza4sec_nuovo.pdf}
\centering
\end{figure}

\begin{figure}
\caption{Funzione di verosimiglianza per $\Delta t = \SI{600}{s}$}
...
\centering
\includegraphics[scale=0.8]{verosimiglianza_comb.eps}
\end{figure}
...
\label{fig:lollo}
\centering
\includegraphics[scale=0.7]{lollo_new.eps}
\end{figure}
